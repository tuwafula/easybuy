...
const HomeScreen = ({ navigation, route }) => {
  ...
  const fetchProduct = async () => {
    try {
      const res = await fetch(`${network.serverip}api/products`, headerOptions);
      ...
      if (data) {
        ...
        data.forEach((cat, index) => {
          let searchableItem = { ...cat, id: ++index, name: cat.name };
    ...
    // fetch(`${network.serverip}/api/products`, headerOptions) //API call
              ...
              <FlatList
                ...
                renderItem={({ item, index }) => (
                  ...
                  >
                    <ProductCard
                      name={item.name}
                      image={`https://res.cloudinary.com/dz9wzvgbd/${item.image}`}
                      price={item.price}
