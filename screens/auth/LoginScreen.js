...
const LoginScreen = ({ navigation }) => {
  ...
  const loginHandle = async () => {
    ...
    try {
      const res = await fetch(`${network.serverip}auth/login/`, requestOptions);
      ...
      if (data) {
        if (data?.userType == "ADMIN") {
          ...
          _storeData(data.jwt);
          setIsloading(false);
          navigation.replace("dashboard", { authUser: data.jwt }); // navigate to Admin Dashboard
        } else {
          _storeData(data.jwt);
          setIsloading(false);
          navigation.replace("tab", { user: data.jwt }); // navigate to User Dashboard
        }
      } else {
        setIsloading(false);
